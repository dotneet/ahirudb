...
//!
//! The test data is real Parquet written by DuckDB. Unit tests use hand-built byte strings,
//! so this file guarantees verification against "what a real writer actually produces".
//! Expected values are decided by cross-checking against the output of `duckdb -c "SELECT ..."`.
...
/// Reads the whole file, column by column.
fn read_all(bytes: &[u8]) -> (Vec<String>, Vec<Vector>) {
    ...
    for rg in &f.meta.row_groups {
        for (i, desc) in f.schema.columns.iter().enumerate() {
            ...
            match &mut cols[i] {
                // Concatenate each RowGroup's result vertically.
                Some(acc) => {
...
fn basic_file_matches_duckdb_output() {
    ...
    // Also check the tail. Mishandling page boundaries would throw this off.
...
fn nulls_land_on_the_right_rows() {
    ...
    for i in 0..1000 {
        ...
        assert_eq!(big.value_at(i) == Value::Null, expect_null, "row {i}'s NULL judgment is off");
    }
...
fn dictionary_encoded_strings_round_trip() {
    let bytes = data("basic.parquet");
    let (_, cols) = read_all(&bytes);
    // `name` is PLAIN_DICTIONARY. 7 distinct values cycle through.
    for i in 0..1000 {
        let expect = alloc_name(i % 7);
        assert_eq!(cols[1].value_at(i), Value::Bytes(expect), "row {i}");
    }
}
...
fn multiple_row_groups_are_concatenated_in_order() {
    ...
    let f = open_bytes(&bytes).unwrap();
    assert!(f.num_row_groups() > 1, "must be a file with multiple RowGroups");
    ...
    // Do order and values hold up across a RowGroup boundary?
...
// duckdb -c "DESCRIBE SELECT * FROM 'zstd.parquet'" -> id INTEGER only, 0..4999

/// With the `zstd` feature on (enabled by default), the core can decompress ZSTD-compressed
/// pages on the spot, without going through host delegation (`NeedCodec`).
#[cfg(feature = "zstd")]
#[test]
...
/// With the `zstd` feature off, it's a clear `UnsupportedCodec` requiring host delegation as
/// before (better than silently breaking).
#[cfg(not(feature = "zstd"))]
#[test]
fn zstd_is_rejected_with_a_clear_code_when_the_feature_is_off() {
    let bytes = data("zstd.parquet");
    let f = open_bytes(&bytes).expect("the footer is uncompressed, so this should open fine");
    ...
    assert_eq!(code, Some(201), "should return UnsupportedCodec");
}

#[test]
fn truncated_file_is_rejected_without_panicking() {
    let bytes = data("basic.parquet");
    // Truncate to a length where the footer cannot be read.
    for cut in [0usize, 4, 8, 100, 1000] {
        let r = open_bytes(&bytes[..cut.min(bytes.len())]);
        assert!(r.is_err(), "a file cut off at {cut} bytes should be rejected");
    }
}
...
fn corrupted_footer_bytes_never_panic() {
    let bytes = data("basic.parquet");
    // Corrupting the footer region's bytes one at a time should never panic; it should just end in an Err or succeed cleanly.
    let n = bytes.len();
    for i in (n - 400)..(n - 8) {
